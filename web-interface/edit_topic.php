		...
		<script>
			$(document).ready(function(){
				...
				function getTopicEdit(){
					...
					$("#t_def").val(String('<?php echo addcslashes($topicEditResult['definition'],"()\n\t");?>'));
					$("#t_obj").val(String('<?php echo addcslashes($topicEditResult['objective'],"()\n\t");?>'));
				}
						...
						$.post("itembank_query.php", {checkDup_topic:dataset}, function(result){
							...
							if(isDupName){
								...
								$("#preTopicModal").html("<div class='row'><div class='col-md-12'><p>Course Name : </p>"+$("#sub_id option[value='"+$("#sub_id").val()+"']").text()+"</div></div>"
												...
												+"<div class='row'><div class='col-md-12'><p>Objective : </p>"+$("#t_obj").val()+"</div></div>");
